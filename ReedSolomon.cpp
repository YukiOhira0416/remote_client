    ...
    std::vector<uint8_t>& decodedData)
{
    ...
    std::vector<uint8_t> encode_matrix(static_cast<size_t>(n) * static_cast<size_t>(k));
    ...
    auto fill_generator_row = [&](int shard_index, uint8_t* dest) {
        if (shard_index < k) {
            ...
            std::memcpy(dest, encode_matrix.data() + static_cast<size_t>(shard_index) * static_cast<size_t>(k), static_cast<size_t>(k));
        }
    };
