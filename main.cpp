        ...
        if (g_parsedShardQueue.try_dequeue(parsedInfo)) {
            ...
            if (tryDecode && !shardsForDecodeAttempt.empty()) {
                ...
                if (g_matrix_initialized.load() && DecodeFEC_Jerasure(std::map<uint32_t, std::vector<uint8_t>>(shardsForDecodeAttempt.begin(), shardsForDecodeAttempt.end()), RS_K, RS_M, originalLenForDecode, *decodedFrameData, g_jerasure_matrix)) {
                    ...
                    if(count % 60 == 0)DebugLog(L"Server FEC Worker Start to Client FEC Worker End Process Time: " + std::to_wstring(elapsed_fec_worker_thread) + L" ms");
                } else {
...
#if defined(_WIN32) && defined(CUDA_VERSION) && CUDA_VERSION >= 10010 && 0 // Disabled problematic cuDeviceGetByLuid call to ensure build compatibility
    {
        CUdevice tmpDev = 0;
