...
void AudioPlaybackThread() {
    ...
    std::optional<AudioFormatPayload> current_format;

    // Block until the first format packet is received
    DebugLog(L"[AudioPlayback] Waiting for initial audio format packet...");
    while (g_audioThreadsRunning && !current_format.has_value()) {
        AudioFormatPayload format_payload;
        if (g_audioFormatQueue.try_dequeue(format_payload)) {
            current_format = format_payload;
            DebugLog(L"[AudioPlayback] Received initial audio format.");
        } else {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
    }

    if (!g_audioThreadsRunning) {
        CoUninitialize();
        return;
    }
    ...
    while (g_audioThreadsRunning) {
        ...
        // Configure WAVEFORMATEX based on the current format
        uint32_t sample_format = ntohl(current_format->sample_format_be);
        if (sample_format == 1) { // 1 = float32
            desiredFormat.wFormatTag = WAVE_FORMAT_IEEE_FLOAT;
        } else { // 2 = int16 (or other PCM)
            desiredFormat.wFormatTag = WAVE_FORMAT_PCM;
        }
        desiredFormat.nSamplesPerSec = ntohl(current_format->sample_rate_be);
        desiredFormat.nChannels = ntohs(current_format->channels_be);
        desiredFormat.wBitsPerSample = ntohs(current_format->bits_per_sample_be);
        ...
        DebugLog(L"[AudioPlayback] Attempting to initialize with format: %s, %lu Hz, %u-bit, %u channels",
            (desiredFormat.wFormatTag == WAVE_FORMAT_IEEE_FLOAT ? L"Float32" : L"PCM"),
            desiredFormat.nSamplesPerSec, desiredFormat.wBitsPerSample, desiredFormat.nChannels);


        CComPtr<IMMDevice> device;
        hr = enumerator->GetDefaultAudioEndpoint(eRender, eConsole, &device);
        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to get default audio endpoint. Retrying...");
        ...
        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to activate audio client. Retrying...");
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            continue;
        }
        ...
        if (hr == S_FALSE) {
            DebugLog(L"[AudioPlayback] Exact format not supported. Using closest match provided by system.");
            // If the system provides a closest match, we should use it.
             hr = audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 10000000, 0, closestMatch, nullptr);
             if (closestMatch) {
                // Update desiredFormat to reflect what's actually being used.
                memcpy(&desiredFormat, closestMatch, sizeof(WAVEFORMATEX));
                CoTaskMemFree(closestMatch);
             }
        } else if (SUCCEEDED(hr)) {
            DebugLog(L"[AudioPlayback] Exact format is supported.");
            hr = audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 10000000, 0, &desiredFormat, nullptr);
        }

        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to initialize audio client. Retrying...");
        ...
        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to get buffer size. Re-initializing...");
        ...
        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to get render client. Re-initializing...");
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            continue;
        }
        ...
        if (FAILED(hr)) {
            DebugLog(L"[AudioPlayback] Failed to start audio client. Re-initializing...");
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            continue;
        }
        ...
        // --- Playback loop ---
        while (g_audioThreadsRunning) {
            AudioFormatPayload new_format_payload;
            if (g_audioFormatQueue.try_dequeue(new_format_payload)) {
                DebugLog(L"[AudioPlayback] New audio format received. Re-initializing.");
                current_format = new_format_payload;
                break; // Exit playback loop to re-initialize client
            }
            ...
            if (!isPlaying && total_buffered_frames >= targetJitterFrames) {
                ...
                isPlaying = true;
            } else if (isPlaying && total_buffered_frames == 0) {
            ...
            if (isPlaying) {
                ...
                if (FAILED(hr)) {
                    DebugLog(L"[AudioPlayback] Failed to get padding. Re-initializing.");
                    break;
                }
                ...
                if (framesAvailable > 0 && !jitterBuffer.empty()) {
                    ...
                    if (g_lastVideoTimestamp > 0) {
                        ...
                        int64_t audio_play_head_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count() +
                                                   (padding * 1000000000ULL / desiredFormat.nSamplesPerSec);
                    ...
                    if (framesToWrite > 0) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            pcmReadOffset += framesToWrite * desiredFormat.nBlockAlign;
                             ...
                             DebugLog(L"[AudioPlayback] GetBuffer failed. Re-initializing.");
                             break;
                        }
                    }
            ...
            // If not playing or buffer is empty, wait a bit
