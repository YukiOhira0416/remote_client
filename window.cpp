...
void RenderFrame() {
    ...
    {
        uint64_t now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(frameEndTime.time_since_epoch()).count();
        uint64_t wgc_ts = renderedFrameData.wgcCaptureTsMs;

        // --- Latency logging with sanity checks ---
        // Only log if the WGC timestamp is valid and in the past.
        if (wgc_ts > 0 && now_ms > wgc_ts) {
            int64_t latencyMs = static_cast<int64_t>(now_ms) - static_cast<int64_t>(wgc_ts);

            // Suppress logging for extremely large values (e.g., > 120 seconds), which indicates an error.
            if (latencyMs < 120000) {
                if(RenderCount++ % 60 == 0) {
                ...
                // Still increment count to avoid spamming other logs if this happens frequently.
                RenderCount++;
            }
        } else {
            // Increment count even on suppressed logs to maintain the modulo behavior.
            RenderCount++;
        }
    }
